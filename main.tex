...
They are not viewpoint invariant as the \gls{bearing-angle} changes when the sensing lightray hits the same surface from a different angle.
...
Both, the pointcloud and the depth image, are first converted to a gray-scale image. 
Instead of just using the distance present in depth-images these new images encode the local geometry, called feature-images.
