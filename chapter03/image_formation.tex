...
\begin{figure}[H]
    ...
    \caption[Spherical camera model and equirectangular images]{\emph{Spherical camera model and equirectangular images.} Each cartesian point that is not the origin can be uniquely converted to spherical coordinates. The origin $\mathbf{O}$ is interpreted as an invalid measurement in the context of this thesis and therefore filtered out during processing.}\label{fig:spherical}
\end{figure}
...
\begin{aligned}
    \vec{\hat{p}} &= \colvecxxx{x}{y}{1} = \frac{1}{Z}\colvecxxx{X}{Y}{Z} \\
    \implies Z \vec{\hat{p}} &= \vec{P} \implies Z \lnorm{\vec{\hat{p}}} &= \lnorm{\vec{P}} \\
    \iff Z r &= d
\end{aligned}
\end{equation}
